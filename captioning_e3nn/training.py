...
def main():
    ...
    # Build data loader
    data_loader = get_loader(configuration, 
                             vocab,
    ...
    decoder = MyDecoderWithAttention(attention_dim = attention_dim, embed_dim = emb_dim,
                                    decoder_dim = decoder_dim, vocab_size = len(vocab),
                                    encoder_dim = encoder_dim, dropout = dropout).to(device).double()
    ...
    for epoch in range(num_epochs):
        for i, (features, geometry, captions, lengths) in enumerate(data_loader):
            ...
            # Print log info
            if i % log_step == 0:
                result = ('Epoch [{}/{}], Step [{}/{}], Loss: {:.4f}, Perplexity: {:5.4f}'
