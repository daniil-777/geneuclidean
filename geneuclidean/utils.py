...
class Utils:
    ...
    def __init__(self, path_root):
        self.init_pocket = path_root + "/data/new_refined/"
        self.init_ligand = path_root + "/data/new_refined/"
        # self.target_casf = path_root + "/core_processed_dataset"
        self.target_casf = path_root + "/data/new_core_2016"
        # self.init_test_data = path_root + "/CASF/PDBbind_core_set_v2007.2.lst"
        self.core_labels_data = path_root + "/data/CASF-2016/power_scoring/CoreSet.dat"
        self.refined_labels_data = (
            path_root + "/data/refined-set/index/INDEX_refined_data.2019"
        )

        # self.init_pocket = path_pocket
        # self.init_ligand = path_ligand
        self.files_refined = os.listdir(self.init_pocket)
    ...
    def plot_statistics(
        self, path: str, path_dest: str, n_epoches: int, name: str, flag: str
    ):
        ...
        pyplot.scatter(target, predict, label="predicted pkd & target")
        plt.xlabel("target_y")
        plt.ylabel("predicted_y")
        plt.legend(loc="upper left")
        plt.title(name + " " + flag + " Pearson correlation={}".format(str(corr)[0:6]))
    ...
    def _get_train_data_special(self):
        files_refined = os.listdir(self.init_pocket)
    ...
    def _get_id_labels(self, path: str):
        ...
        """
        file = open(os.path.join(self.init_ligand, path), "r")
        lines = file.readlines()
