...
class Pdb_Dataset(Dataset):
    ...
    def _get_elems(self, protein_id: int, type_filtering: str):
        ...
        try:
            ...
            if type_filtering == "filtered":
                mol_pocket_element = [
                    elem for elem in mol_pocket.element if elem in self.common_atoms
                ]
                mol_ligand_element = [
                    elem for elem in mol_ligand.element if elem in self.common_atoms
                ]
    ...
    def _get_feature_vector_atom(self, elem: str, type_atom: str, type_filtering: str):
        ...
        if type_atom == "pocket":
            ...
            if type_filtering == "filtered":
                ...
                feature_vector_atom = np.concatenate(
                    (self.label_ligand, hot_vector_atom)
                )
                feature_vector_atom = np.array([hot_vector_atom])
    ...
    def _get_features_complex(self, id: int):
        ...
        features_ligand_part = self._get_features_unit(
            elem_ligand, "ligand", self.type_filtering
        )
        features_all = features_pocket_part + features_ligand_part
        tensor_all_features = torch.tensor(features_all, dtype=torch.long).unsqueeze(0)
    ...
    def _get_coord(self, protein_id: int, type_filtering: str):
        ...
        if type_filtering == "all":
            ...
            lig_idxs = [
                idx
                for idx, elem in enumerate(mol_ligand.element)
                if elem in self.common_atoms
            ]
            coords_ligand = [
                element for i, element in enumerate(mol_ligand.coords) if i in lig_idxs
            ]
